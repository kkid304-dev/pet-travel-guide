...
def _size_verdict(text, pet_weight, pet_size):
    ...
    "대형견 제외"라는 정확한 문구만 찾는 좁은 규칙과, 크기 단어가 있으면 무조건 "그
    크기만 허용"으로 읽는 넓은 규칙이 따로 있으면, 후자가 "대형견 제외 전 견종 가능"
    처럼 배제 의미로 쓰인 크기 단어까지 허용 목록으로 오독한다. 또 "소형견 전용...
    대형견 불가" 같은 문장은 배제 대상이 좁은데(대형만) 넓은 "불가 있고 가능 없음"
    규칙에 먼저 걸려 전체가 배제로 오판될 수 있다. 그래서 배제/허용을 먼저 분리해서
    보고, 배제 대상이 아니면 남은 무게 조건까지 확인한 뒤에야 확정한다."""
    ...
    # 내 반려견이 바로 그 애매한 크기라면 확답 대신 유보한다.
...
def _judge_core(text, pet_weight, is_dangerous_breed):
    ...
    if "맹견" in text:
        ...
        # 무게 조건을 확인하고 그 결과와 맹견 여부를 같이 반영한다.
        weight_verdict = _weight_verdict(text, pet_weight)
        if weight_verdict is not None:
